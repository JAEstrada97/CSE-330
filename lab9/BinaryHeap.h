...
template<typename C>
BinaryHeap<C>::BinaryHeap(const Vector<C>& items): heap(items.size + 10),
...
template<typename C>
void BinaryHeap<C>::printHeap() {
    printHeap(1,0);
}

template<typename C>
void BinaryHeap<C>::printHeap(int start, int offset) const {
    ...
    printHeap(2*start,offset + 1);
    printHeap((2*start) + 1, offset + 1);
}
